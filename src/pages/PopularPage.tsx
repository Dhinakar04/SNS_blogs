...
const tabs = ["Past Week", "Past Month", "Past Year"];

const PopularPage = () => {
  const { data: articles = [], isLoading } = useArticles();
  const [activeTab, setActiveTab] = useState("Past Month");
  ...
  const filteredArticles = articles.filter(a => {
    ...
    if (activeTab === "Past Week") return diffDays <= 7;
    if (activeTab === "Past Month") return diffDays <= 30;
    if (activeTab === "Past Year") return diffDays <= 365;
    return true;
  });

  const popular = [...filteredArticles].sort((a, b) => b.likes - a.likes);
    ...
    <Layout>
      <div className="container-blog section-spacing">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl">Most Popular</h1>
          </div>
          ...
          <div className="flex flex-col">
            {popular.length === 0 ? (
              <p className="text-muted-foreground text-center py-20">No articles found in this time range.</p>
            ) : (
              popular.map((a, i) => (
                <Link
                  key={a.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-3">
                      <p className="metadata">{a.date} · {a.readTime} read</p>
                      <span className="flex items-center gap-1 text-accent">
                        <Heart className="w-3 h-3 fill-accent" />
                        <span className="text-xs font-medium">{a.likes}</span>
                      </span>
                    </div>
                  </div>
